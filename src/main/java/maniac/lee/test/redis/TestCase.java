...
//		sortingParameters.get("#");
    ...
    public void testSortedSetUsage() {
        Jedis jedis = RedisUtil.getJedis();
        String key = "zhongsou:hackers";
        jedis.zadd(key, 1940, "Alan Kay");
        jedis.zadd(key, 1953, "Richard Stallman");
        jedis.zadd(key, 1943, "Jay");
        jedis.zadd(key, 1920, "Jellon");
        jedis.zadd(key, 1965, "Yukihiro Matsumoto");
        jedis.zadd(key, 1916, "Claude Shannon");
        jedis.zadd(key, 1969, "Linus Torvalds");
        jedis.zadd(key, 1912, "Alan Turing");

        Set<String> hackers = jedis.zrange(key, 0, -1);
        System.out.println(hackers);

        Set<String> hackers2 = jedis.zrevrange(key, 0, -1);
        System.out.println(hackers2);

        // 区间操作,我们请求Redis返回score介于负无穷到1920年之间的元素（两个极值也包含了）。
        Set<String> hackers3 = jedis.zrangeByScore(key, "-inf", "1920");
        System.out.println(hackers3);
        System.out.println(jedis.zrangeByScore(key, "-inf", "1920"));

        // ZREMRANGEBYSCORE 这个名字虽然不算好，但他却非常有用，还会返回已删除的元素数量。
        long num = jedis.zremrangeByScore(key, "-inf", "1920");
        System.out.println(num);
        System.out.println(jedis.zremrangeByScore(key, "-inf", "1920"));
